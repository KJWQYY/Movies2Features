...
parser.add_argument('--feature_root', type=str, default="../features", help='path to features')

parser.add_argument('--cache_dir', type=str, default="cache/checkpoints/clip_cache")
#parser.add_argument('--cache_dir', type=str, default="cache/checkpoints/cn_clip_cache")
...
if args.clip_version=="eva_clip":
    ...
    # model, preprocess = cn_clip.load_from_name(args.model_version, device=device, download_root=args.cache_dir)
    # model = model.to(device)
model.eval()
...
with open(join(args.feature_root,"key_frame_{}_L.txt".format(args.phase)),"r") as f:
    data = f.readlines()
SAVE_FOLDER = join(args.feature_root,"Visual_features_L")
...
for index in tqdm(range(len(data)), desc='extract clip features'):
    ...
    assert shot_frame0_path == shot_path
    shot_frame0_pil = Image.open(shot_frame0_path)
    try:
        shot_frame1_pil = Image.open(shot_frame1_path)
    except:
        print("special shot, without 1") # shot is too short, only 2 frames within the shot
        special_shot+=1
        shot_frame1_pil = Image.open(shot_frame0_path)
    try:
        shot_frame2_pil = Image.open(shot_frame2_path)
    except:
        print("special shot, without2") # shot is too short, only 2 frames within the shot
        special_shot+=1
        shot_frame2_pil = Image.open(shot_frame0_path)
    ...
    # feat_dict[shot_frame0_path] = shot_frame0_feats
    # feat_dict[shot_frame1_path] = shot_frame1_feats
    # feat_dict[shot_frame2_path] = shot_frame2_feats
    feat_dict[shot_frame0_path.replace(args.shotdata_path+"/","")] = shot_frame0_feats
    feat_dict[shot_frame1_path.replace(args.shotdata_path+"/","")] = shot_frame1_feats
    feat_dict[shot_frame2_path.replace(args.shotdata_path+"/","")] = shot_frame2_feats
    # break
